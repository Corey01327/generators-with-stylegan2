...
def submit_run(submit_config: SubmitConfig, run_func_name: str, **run_func_kwargs) -> None:
    ...
    if not re.match(docker_valid_name_regex, submit_config.task_name):
        raise RuntimeError(
            f"Invalid task name.  Probable reason: unacceptable characters in your submit_config.run_desc.  Task name must be accepted by the following regex: {docker_valid_name_regex}, got {submit_config.task_name}"
        )
